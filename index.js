...
// 🔥🔥 ROBOT İÇİN EKLENEN KISIM (404 HATASINI BU ÇÖZECEK) 🔥🔥
app.get('/', (req, res) => {
    res.send("Merak etme, sunucu ayakta! 🚀");
});
...
// 🔥 DÜZELTİLDİ: Timeout sorunu için 10 saniye yapıldı
const pool = new Pool({
...
app.get('/bolumler', async (req, res) => { 
    const r = await pool.query('SELECT DISTINCT fakulte, bolum FROM dersler ORDER BY fakulte, bolum'); 
    res.json(r.rows); 
});

app.get('/hocalar', async (req, res) => { 
    const r = await pool.query("SELECT DISTINCT hoca_adi FROM dersler WHERE hoca_adi != 'Belirtilmemiş' ORDER BY hoca_adi"); 
    res.json(r.rows); 
});

app.get('/dersler/:bolum', async (req, res) => { 
    const r = await pool.query('SELECT * FROM dersler WHERE bolum = $1', [req.params.bolum]); 
    res.json(r.rows); 
});

app.get('/hoca-dersleri/:hoca', async (req, res) => { 
    const r = await pool.query('SELECT * FROM dersler WHERE hoca_adi = $1', [req.params.hoca]); 
    res.json(r.rows); 
});
...
app.post('/iletisim-gonder', async (req, res) => { 
    await pool.query('INSERT INTO iletisim_mesajlari (mesaj) VALUES ($1)', [req.body.mesaj]); 
    res.json({ success: true }); 
});

app.get('/forum/:tur', async (req, res) => { 
    const ana = await pool.query('SELECT * FROM forum WHERE tur = $1 AND ust_id = 0 ORDER BY tarih DESC', [req.params.tur]); 
    const cev = await pool.query('SELECT * FROM forum WHERE tur = $1 AND ust_id != 0 ORDER BY tarih ASC', [req.params.tur]); 
    const sonuc = ana.rows.map(s => ({ ...s, cevaplar: cev.rows.filter(c => c.ust_id === s.id) })); 
    res.json(sonuc); 
});

app.post('/forum-ekle', async (req, res) => { 
    await pool.query('INSERT INTO forum (tur, ust_id, kullanici_adi, mesaj) VALUES ($1, $2, $3, $4)', [req.body.tur, req.body.ust_id||0, req.body.kullanici_adi, req.body.mesaj]); 
    res.json({ success: true }); 
});
...
app.post('/yorum-sil', async (req, res) => { 
    try { 
        ...
        console.log(`Silme İsteği: ${tur} - ID: ${id} - Kim: ${kullanici_adi}`); 
...
app.delete('/admin/sil-mesaj/:id', async (req, res) => { 
    await pool.query(`DELETE FROM iletisim_mesajlari WHERE id=$1`, [req.params.id]); 
    res.json({ success: true }); 
});

app.post('/admin/banla', async (req, res) => { 
    await pool.query("UPDATE users SET is_banned = true WHERE nickname = $1", [req.body.nickname]); 
    res.json({ success: true }); 
});
