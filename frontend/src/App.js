  ...
  return ( 
    <div className="modal-overlay"> 
      <div className="modal-box"> 
        ...
        <h2 style={{ textAlign: 'center', color: '#004aad', marginTop: 0 }}>
            {tip === 'giris' ? 'Giriş Yap' : (kayitAsama === 1 ? 'Kayıt Ol' : 'Doğrulama')}
        </h2> 
            ...
            kayitAsama === 1 ? ( 
              ...
              <> 
                <div style={{display:'flex', alignItems:'center', marginBottom:5}}>
                    <button onClick={()=>setKayitAsama(1)} style={{background:'none', border:'none', cursor:'pointer', fontSize:'18px'}}>⬅️</button>
                    <p style={{margin:'0 auto', fontSize:'0.9em'}}>Kodu gir:</p>
                </div>
                <input type="text" placeholder="123456" value={code} onChange={e => setCode(e.target.value)} className="modal-input" maxLength={6} style={{textAlign:'center', letterSpacing:5}}/> 
                <input type="text" placeholder="Takma Ad" value={nickname} onChange={e => setNickname(e.target.value)} className="modal-input" autoComplete="off" /> 
                <input type="password" placeholder="Şifre Belirle" value={password} onChange={e => setPassword(e.target.value)} className="modal-input" autoComplete="off" /> 
                <button onClick={kayitTamamla} className="modal-btn" style={{background:'#28a745'}}>Kayıt Ol</button> 
              </> 
            ) 
          )} 
        </div> 
...
function YurtlarSayfasi() {
  ...
  const yurtSec = (yurt) => { setSeciliYurt(yurt);
  fetch(`${API_URL}/yurt-yorumlari/${yurt}`).then(res => res.json()).then(data => { if(Array.isArray(data)) setYorumlar(data); else setYorumlar([]); }).catch(()=>setYorumlar([])); };
  const yorumGonder = () => { if (!yeniYorum.trim()) return;
  fetch(`${API_URL}/yurt-yorum-ekle`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ yurt_adi: seciliYurt, yorum_metni: yeniYorum, kullanici_adi: kullanici.nickname }) }).then(() => { setYeniYorum(""); yurtSec(seciliYurt); });
  };
...
function DersDetay() {
  ...
  const yorumGonder = () => { if (!yeniYorum.trim()) return; if (!kullanici) { alert("Giriş yapmalısın!"); return;
  } fetch(`${API_URL}/ders-yorum-ekle`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ders_kodu: ders.ders_kodu, ders_adi: ders.ders_adi, kullanici_adi: kullanici.nickname, yorum_metni: yeniYorum }) }).then(() => { setYeniYorum(""); verileriGuncelle(); });
  };
