...
function AnaSayfa() {
  ...
  return (
    <div className="main-container">
      ...
      {/* MASAÜSTÜ HEADER (ORTALANDI) */}
      <header className="desktop-header" style={{textAlign: 'center'}}> 
      ...
      <div className="content-grid">
        {/* SOL (Menüler Alt Alta) */}
        <div className="left-col desktop-only"> 
            <h3 className="col-title">Menü</h3> 
            {kullanici && kullanici.nickname === 'baraykanat' && <div onClick={() => navigate('/admin')} className="menu-item admin-btn">👑 Admin Paneli</div>} 
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}> 
                {menuler.map(m=><div key={m.id} onClick={()=>navigate(m.l)} className="menu-item"><span style={{marginRight:12, fontSize:18}}>{m.i}</span>{m.t}</div>)} 
            </div> 
        </div>
        
        {/* ORTA */}
        <div className="center-col"> {!kullanici ? (<> <h2 style={{ color: '#004aad', fontSize: '26px', margin: '0 0 15px 0' }}>Hoş Geldin</h2> <p style={{ color: '#555', marginBottom: '30px' }}>Yorum yapmak için giriş yapmalısın.</p> <div style={{display:'flex', flexDirection:'column', gap:12, maxWidth:220, margin:'0 auto'}}> <button onClick={()=>{setModalTip('giris');setModalAcik(true)}} className="login-btn">Giriş Yap</button> <button onClick={()=>{setModalTip('kayit');setModalAcik(true)}} className="register-btn">Kayıt Ol</button> </div> </>) : (<> <h2 style={{ color: '#004aad', fontSize: '26px', margin: '0 0 10px 0' }}>{kullanici.nickname}</h2> <p style={{ color: '#555', marginBottom: '30px' }}>Giriş yaptın.</p> <button onClick={cikisYap} className="logout-btn">Çıkış Yap</button> </>)} <div className="donation-bar-container"> <p className="donation-text">2025 31 Aralık tarihine kadar her 600 yorum için<br/>Darüşşafaka'ya 200 TL Bağış!</p> <div className="progress-bg"> <div className="progress-fill" style={{ width: `${barYuzdesi}%` }}></div> </div> <small style={{ color: '#777' }}>{toplamYorum} / 600 Yorum</small> </div> </div>
        
        {/* SAĞ */}
        <div className="right-col desktop-only"> <h3 className="col-title">İletişim</h3> <div className="iletisim-box"> <p style={{ fontSize: '14px', color: '#555', lineHeight: '1.5', marginBottom: '15px', marginTop: 0 }}>Tavsiye ve önerileriniz için:</p> {!iletisimAcik ? <button onClick={()=>setIletisimAcik(true)} className="msg-btn">Mesaj Yaz</button> : <div><textarea className="msg-input" rows="4" value={mesaj} onChange={e=>setMesaj(e.target.value)}/><button onClick={mesajGonder} className="send-btn">Gönder</button></div>} {bilgi && <div style={{color:'green', marginTop:5, fontSize:'0.9em', textAlign:'center'}}>{bilgi}</div>} <div style={{marginTop:15, paddingTop:15, borderTop:'1px solid #eee', textAlign:'center'}}><a href="mailto:cukampus2025@gmail.com" style={{color:'#004aad', textDecoration:'none', fontWeight:'bold'}}>📧 Mail At</a></div> </div> </div>
      </div>
    </div>
  );
}

// --- HOCALAR SAYFASI (FULL LİSTE VE ARAMA) ---
