...
function GirisModal({ kapali, kapat, tip }) { 
  ...
  const kodGonder = async () => { 
      ...
      try { 
          ...
          const data = await res.json(); 
          if (!data.success) { setHata(data.error); setBilgi(""); } 
          else { setBilgi("✅ Kod mailine gönderildi!"); setKayitAsama(2); } 
  ...
  return ( <div className="modal-overlay"> <div className="modal-box"> <button onClick={kapat} className="modal-close">✖</button> <h2 style={{ textAlign: 'center', color: '#004aad', marginTop: 0 }}>{tip === 'giris' ? 'Giriş Yap' : 'Kayıt Ol'}</h2> {hata && <div className="modal-error">⚠️ {hata}</div>} {bilgi && <div className="modal-info" style={{color:'green', textAlign:'center', marginBottom:10}}>{bilgi}</div>}
...
const AdminPanel = () => {
    ...
    useEffect(() => { veriCek(); }, []);
    const sil = (tur, id) => { if(!window.confirm("Silmek istediğine emin misin?")) return; fetch(`${API_URL}/admin/sil/${tur}/${id}`, { method: 'DELETE' }).then(() => { veriCek(); }); };
    const banla = (nickname) => { if(!nickname || nickname === 'Anonim') return; if(window.confirm(`DİKKAT: ${nickname} banlansın mı?`)) { fetch(`${API_URL}/admin/banla`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ nickname }) }).then(() => alert("Kullanıcı Banlandı!")); } };
